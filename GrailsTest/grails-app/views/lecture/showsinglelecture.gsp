...
<html>
	<head>
		<meta name="layout" content="lecturemain">
...
<%--				</g:if>--%>
				...
				<li class="fieldcontain">
					<span id="stream-label" class="property-label"><g:message code="lecture.stream.label" default="Streams" /></span>
						...
						<span class="property-value" aria-labelledby="stream-label">${lectureInstance?.stream?.coursename?.encodeAsHTML()}</span>
					
				</li>
				</g:if>
				
				<g:link controller="lectureTimeTable" action="showTimeTable1" id="${lectureInstance.id}">Click To see time table </g:link>
