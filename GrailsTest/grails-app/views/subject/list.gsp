			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="year" title="${message(code: 'subject.year.label', default: 'Year')}" />
					
						<th><g:message code="subject.stream.label" default="Streams" /></th>
				...
				<g:each in="${subjectInstanceList}" status="i" var="subjectInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td>${fieldValue(bean: subjectInstance, field: "year")}</td>
					
						<td>${fieldValue(bean: subjectInstance, field: "stream.coursename")}</td>
